...
Sweet is an external dependency, pinned to the `v0.3.1` release as a git dependency in `Cargo.toml` (local development can override it back to `../sweet/crates/` via the `[patch]` section). **Do not add sweet-* crates as members of this workspace.** Treat the sweet crate boundary as a repo boundary — no reaching across without a proper API.
